...
def crop_fish(image, bbox):
    ...
    return cv2.resize(crop, (CLS_IMGSZ, CLS_IMGSZ))


def analyze_image(image, seg_model, cls_model):
    """단일 이미지 분석 → 결과 JSON"""
    # Stage 1: 물고기 탐지
    seg_results = seg_model(image, verbose=False)
    fish_results = []

    if len(seg_results) == 0 or seg_results[0].boxes is None:
    ...
    for i, box in enumerate(boxes):
        ...
        fish_results.append({
            ...
            "seg_confidence": round(seg_conf, 3),
            "symptom": cls_name,
...
def analyze_video(video_path, seg_model, cls_model, frame_interval=30):
    """영상 분석 (N 프레임마다 분석)"""
    ...
    while True:
        ...
        if frame_idx % frame_interval == 0:
            result = analyze_image(frame, seg_model, cls_model)
...
def draw_results(image, result):
    ...
    tank_color = colors.get(tank_level, (255, 255, 255))
...
def main():
    ...
    if args.image:
        ...
        results = analyze_video(args.video, seg_model, cls_model, args.frame_interval)
